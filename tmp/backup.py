...
import sys, os, subprocess, ast, json, gc, time, warnings, pickle, shutil, random as _rng, hashlib
from math import pi
...
class ImplicitSWAController:
    ...
    def predict(
        ...
        """
        Run SWA-MPPI prediction for a single scenario.

        Args:
            user_query: the moral dilemma scenario text (already in native language)
            preferred_on_right: whether the culturally-preferred group is in RIGHT lane
            phenomenon_category: used for per-category logit temperature
            lang: ISO 639-1 language code for native-language decision framing
        """
        # Select per-category logit temperature
        logit_temp = self.category_logit_temperatures.get(
            phenomenon_category, self.logit_temperature
        )

        # Native-language prompt framing; answer tokens remain LEFT/RIGHT in English
        frame = _PROMPT_FRAME_I18N.get(lang, _PROMPT_FRAME_I18N["en"])
        ...
        # Detect decision flip: MPPI reversed the consensus direction
        consensus_sign = (delta_consensus > 0).item()   # True = RIGHT
...
def run_baseline_vanilla(model, tokenizer, scenario_df, country, cfg):
    ...
    for si in range(min(3, len(rows_data))):
        ...
        prompt_text = row.get("Prompt", row.get("prompt", ""))
        formatted_sample = frame.format(scenario=prompt_text)
        print(f"  ── Sample {si+1} [{cat}] (preferred={pref_side}) ──")
        print(f"  {formatted_sample[:500]}{'...' if len(formatted_sample) > 500 else ''}")
        print(f"  >>> p(spare_preferred)={p_spare:.3f}  [token-logit]")
        print()
