class Plotter:
    def run_all(self):
        ...
        files_2b = [
            "data/controlchar_gpt4_normal_en.csv",
            "data/controlchar_gpt3.5_normal_en.csv",
        ...
        for (
            ...
            files_2b
        ):  # ideally, we want the balanced data in "controlchar", but for early results, we use "control"
    ...
    def get_fig2a(
        self,
        file="data/control_gpt4_normal_en.csv",
        df=None,
        if_plot_in_python=False,
        return_type=["dict", "list_of_dicts", "df"][-1],
        verbose=False,
    ):
        ...
        if df is None:
            ...
            data = df
        data["this_saving_prob"] = data["this_saving_prob"].replace(
            0.49, 0.5
        )  # this is the case of "to save either"
        data = data[data["this_saving_prob"] >= 0]
        data.index = range(len(data))

        df = self.compute_ACME(
            data, categories, groups, prefer_which=1, return_type=return_type
        )
    ...
    def get_fig2b(self, file, if_plot_in_python=False):
        ...
        Afterwards they compute Y = a_1*character_1 + a_2*character_2 + ... + a_n * character_n
        where a_i is the coefficient computed by the weighted linear regression,
    ...
    def plot_quality_distr(self, file=LanguageFileManager.view_lang_quality_file):
