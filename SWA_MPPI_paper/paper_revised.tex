...
consensus is high and no correction is applied. An extensive robustness
suite-held-out $\tau$ calibration ($|\Delta\text{JSD}|{=}0.0006$ vs.\
in-distribution), three temperature sweeps (JSD spans $<\!0.010$), six dataset
configurations (range $0.0034$), four deterministic consensus-shift baselines, a
Binary-ARGS adaptation, and an entropy-aware $\sigma$ diagnostic-isolates
\emph{Prospect-Theory asymmetry} as the load-bearing mechanism and rules out
hyperparameter or distributional confounding. Code available at
\texttt{[repository]}.
\end{abstract}
...
\begin{itemize}
  ...
  \item We provide an extensive Round-2 robustness suite that closes the gap to a
        rigorous inference-time alignment study: (a) per-country held-out $\tau$
...
To answer the reviewer question of whether MPPI's importance weighting is
necessary at all, we add two new baseline families that share SWA-MPPI's
...
Table~\ref{tab:robustness_summary} consolidates every robustness experiment we
ran in response to reviewer concerns. Each row reports an empirical claim, the
...
\caption{Round-2 robustness suite. Each test independently probes one assumption
...
alignment is already high. The Anglosphere paradox illustrates how RLHF biases
worsen alignment even for well-represented populations, and a twelve-test
...
enables effective debiasing. To answer the reviewer concern that these choices
could distort the effective evaluation distribution, we evaluate \emph{six}
...
We test whether an entropy-aware adaptive $\sigma$ can address the Qwen2.5-32B
