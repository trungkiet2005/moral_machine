...
\begin{abstract}
% ============================================================
Large language models encode Western moral priors while deployed worldwide---yet
aligning moral decisions to \emph{country-specific} human preferences usually
requires fine-tuning, reward models, or labelled data we do not have at scale.
We ask: can we steer moral choices \emph{at inference time}, on frozen weights,
using only survey-grounded cultural structure? We introduce \textbf{SWA-MPPI}:
World Values Survey personas disagree on contested dilemmas; that disagreement
triggers a Prospect-Theory-weighted Model Predictive Path Integral (MPPI)
update in logit space, with a minimal-KL, importance-weighted correction.
We evaluate on MultiTP across 15 countries, 12 native languages, and six moral
dimensions, spanning \textbf{six} open-weight models from 7B to 126B (Qwen2.5,
Llama~3.1, Mistral-Large). The headline gains are on strong instruction-tuned
70B-class backbones: mean Jensen--Shannon distance to human preferences drops
by \textbf{37.6\%} (Qwen2.5-72B) and \textbf{34.7\%} (Llama-3.1-70B), beating
prompting and activation-steering baselines by $\geq$10\% with one extra
batched forward pass. Smaller Qwen/Llama models improve consistently
(+18--23\%); Mistral-Large improves modestly (+5.8\%) from an already strong
baseline; Qwen2.5-32B yields a negative headline delta that we trace to extreme
logit concentration---a model-specific diagnostic, not a silent failure. The
largest improvements cluster in Anglosphere countries ($+$72--75\% JSD),
suggesting that RLHF-era alignment can still leave substantial mismatch even for
well-represented populations. SWA-MPPI is self-limiting when personas already
agree. Held-out $\tau$ calibration, temperature and dataset sweeps, and
mechanistic baselines isolate \emph{Prospect-Theory asymmetry} as the
load-bearing signal. Code: \texttt{[repository]}.
\end{abstract}
...
\begin{itemize}
  ...
  \item Across six models (7B--126B), we report mean JSD reductions up to
        \textbf{37.6\%} (Qwen2.5-72B) and \textbf{34.7\%} (Llama-3.1-70B) on 15
        countries, with gains reaching $+$75\% for Anglosphere pairs, and
        outperform all baselines by $\geq$10\% on the 70B-class runs
        (\S\ref{sec:results}).
  ...
  \item We provide an extensive robustness suite for inference-time alignment:
        (a) per-country held-out $\tau$
        calibration on 15 countries shows $|\Delta\text{JSD}|{=}0.0006$ vs.\ in-distribution
        calibration (App.~\ref{app:tau_holdout}); (b) three temperature sweeps yield
        JSD spans below $0.010$ with defaults strictly more conservative than the best
        configuration (App.~\ref{app:temp_sensitivity}); (c) six dataset preprocessing
        configurations span only $0.0034$ in JSD (App.~\ref{app:dataset_sens});
        (d) deterministic consensus-shift baselines and a Binary-ARGS adaptation isolate
        \emph{Prospect-Theory asymmetry}, not WVS weighting per~se, as the active
        mechanism (App.~\ref{app:cs_baseline}--\ref{app:args}); (e) an entropy-aware
        adaptive $\sigma$ diagnoses the Qwen2.5-32B failure mode at the logit level
        (App.~\ref{app:adaptive_sigma}); and (f) the pipeline runs end-to-end on 8
        additional countries spanning 4 scripts and 5 language families
        (App.~\ref{app:extended}).
\end{itemize}
...
Recent methods such as DIFFPO-style policy-agnostic aligners, SWAI-style
training-free logit steering, and SLED-style self-logit evolution demonstrate
that post-hoc control at inference time is increasingly practical. Our scope is
more specific: black-box, single-model, binary forced-choice moral decisions
with country-conditioned persona grounding. Relative to structured multi-agent
pipelines such as OG-MAR~\citep{ogmar2024}, SWA-MPPI trades ontology-level
reasoning depth for lower deployment overhead (one additional batched forward
pass and no extra trained aligner).
...
MPPI to random noise. We empirically diagnose this failure mode in
Appendix~\ref{app:adaptive_sigma}: the mean top-50 token entropy on Qwen2.5-32B is
only $H{=}0.521$ nats (vs.\ $H_{\text{ref}}{=}2.5$), and an entropy-aware adaptive
$\sigma$ correctly self-scales ($\bar{\sigma}{=}0.086$, $r(H,\sigma){=}0.948$).
Crucially, the per-country sweep reveals \emph{two latent clusters}:
USA/JPN/GBR/KOR benefit from large $\sigma$ (USA improves $-39\%$ at $\sigma{=}1.0$),
while DEU/CHN/BRA/VNM strictly prefer small $\sigma$. Because all 8 countries share
$H{\approx}0.52$ under Qwen2.5-32B, an entropy signal alone cannot discriminate the
clusters-a complete fix requires \emph{jointly} adaptive $\tau$ per country
(Appendix~\ref{app:adaptive_sigma}, Appendix~\ref{app:limitations}).
...
\paragraph{Isolating the active mechanism: deterministic shifts and Binary-ARGS.}
To test whether MPPI's importance weighting is necessary, we compare against two
new baseline families that share SWA-MPPI's
infrastructure (batched multi-agent forward pass, two-pass debiasing) and differ
only in the decoding rule (Appendices~\ref{app:cs_baseline}--\ref{app:args}).
\textbf{Consensus shifts.} Four deterministic functionals of $\bar{\delta}$
(\textsc{CS-Mean}, \textsc{CS-Std}, \textsc{CS-Scaled}, \textsc{CS-Clamp}) reproduce
B5 PersonaConsensus exactly when uniform ($\bar{\delta}=$ B5; JSD~$0.0590$) and peak
at \textsc{CS-Clamp} (JSD~$0.0532$, $r{=}0.428$); SWA-MPPI still wins
(JSD~$0.0502$). The residual gap of $\sim$0.003~JSD over the best clamp baseline
is attributed to Prospect Theory's asymmetric gain/loss weighting that no scalar
shift can replicate.
\textbf{Binary-ARGS.} We re-implement ARGS~\citep{khanov2024args} for binary
forced-choice with three reward specifications: \textsc{ARGS-Unif} (uniform
directional, $\equiv$ B5), \textsc{ARGS-WVS} (WVS-salience-weighted, no PT), and
\textsc{ARGS-PT} (Prospect Theory utility = SWA-MPPI kernel). Two findings are
striking. First, \textsc{ARGS-WVS} (JSD~$0.0544$) is \emph{worse} than
\textsc{ARGS-Unif} (JSD~$0.0524$) in 12/15 countries: WVS weighting without
asymmetric loss-sensitivity actively misaligns persona contributions. Second,
\textsc{ARGS-PT} (JSD~$0.0485$, $r{=}0.514$) recovers SWA-MPPI in 13/15 countries,
empirically confirming that \emph{SWA-MPPI is precisely an ARGS variant with
KL-regularised Prospect-Theory utility}-and that the Prospect-Theory asymmetry,
not WVS weighting per~se, is the load-bearing component.
...
populations one might expect to be best served. Their high vanilla JSD
($\approx$0.10--0.11) is consistent with model-level moral priors (including
possible RLHF effects, e.g., extreme animal protection and strong
...
best-represented in training data. This pattern suggests that current alignment
pipelines can still encode specific moral priors (e.g., extreme animal
protection and strong gender-neutral stances) that reduce agreement with
country-level human preferences, and SWA-MPPI's WVS-derived personas partially
...
countries constrain applicability. Hyperparameter validation uses a synthetic,
author-annotated set (Appendix~\ref{app:hyperparams}), which may introduce
annotation bias despite transfer checks on a disjoint MultiTP subset. Finally,
although we stress-test multiple preprocessing choices, this version does not yet
include a full ``as-is'' (no capping/upsampling/filtering) MultiTP evaluation,
limiting direct comparability with some prior reports. Extended limitations are in
...
alignment is already high. The Anglosphere paradox indicates that current
alignment pipelines can still leave substantial mismatch even for
well-represented populations, and a twelve-test
robustness suite---held-out $\tau$, three temperature sweeps, six dataset
configurations, four deterministic shift baselines, three Binary-ARGS rewards,
and an entropy-aware $\sigma$ diagnostic---rules out hyperparameter and
distributional confounding while pinpointing Prospect-Theory asymmetry on
KL-regularised perturbations as the load-bearing mechanism. Future directions include:
...
\section{Extended Country Coverage (Generalisation Probe)}
\label{app:extended}

We evaluate the SWA-MPPI pipeline on 8 additional countries not in the MultiTP
benchmark, spanning 4 script systems and 5 language families: Poland (pl),
Turkey (tr), Indonesia (id), Argentina (es), Ukraine (uk), Taiwan (zh),
the Philippines (tl), and Egypt (ar). Because no human AMCEs are available for
these countries, we generate 300 synthetic scenarios per country (50 per moral
dimension); the Vanilla AMCE is uniformly $50.0$ by construction, so this
appendix is a pipeline-generalisation probe rather than an alignment benchmark.
