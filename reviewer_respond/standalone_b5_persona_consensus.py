#!/usr/bin/env python3
"""
STANDALONE — Baseline 5: Persona Consensus — Mean Logit (No MPPI)
===================================================================
Copy toàn bộ file này vào 1 Kaggle cell, chạy độc lập không cần main.py.
Soft consensus: average continuous p_right thay vì discrete vote (B4).
KEY ABLATION: SWA-MPPI − B5 = đóng góp thuần của MPPI control step.
B5 cũng log "mppi_would_fire_rate" để argue trong paper.
"""
...
if _ON_KAGGLE:
    print("[SETUP] Baseline 5 — PersonaConsensus (No MPPI): Installing...")
...
class SWAConfig:
    ...
    tau_conflict: float = 0.001      # used only for diagnostic reporting in B5
...
# ═══════════════════════════════════════════════════════════════════════
# BASELINE 5 SPECIFIC: Persona Consensus Mean Logit (No MPPI)
# KEY ABLATION: SWA-MPPI − B5 = pure MPPI contribution
# ═══════════════════════════════════════════════════════════════════════
@torch.no_grad()
def _b5_batch_forward(model, tokenizer, chat_helper, persona_prefix_ids, query_ids,
                       left_id, right_id, pad_id, logit_temperature, decision_temperature, device):
    seqs = [torch.cat([p, query_ids], dim=1) for p in persona_prefix_ids]
    max_len = max(s.shape[1] for s in seqs)
    batch_ids, batch_mask = [], []
    ...
    lr_logits = logits[:, [left_id, right_id]] / logit_temperature
    p_rights, deltas = [], []
    for i in range(len(persona_prefix_ids)):
        delta = (lr_logits[i,1] - lr_logits[i,0]).item()
        deltas.append(delta)
        p_rights.append(torch.sigmoid(torch.tensor(delta)/decision_temperature).item())
    return p_rights, deltas

def run_b5_persona_consensus(model, tokenizer, scenario_df, country, personas, cfg):
    device      = next(model.parameters()).device
    lang        = _COUNTRY_LANG.get(country, "en")
    chat_helper = ChatTemplateHelper(tokenizer)
    left_id  = tokenizer.encode("LEFT",  add_special_tokens=False)[0]
    ...
    frame    = _PROMPT_FRAME_I18N.get(lang, _PROMPT_FRAME_I18N["en"])
    sf       = _SCENARIO_FRAME_I18N.get(lang, _SCENARIO_FRAME_I18N["en"])
    print(f"\n[B5-PersonaConsensus] {country} (lang={lang}) — {len(personas)} personas, MEAN LOGIT, NO MPPI")
    persona_prefix_ids = [chat_helper.build_prefix_ids(p, device) for p in personas]
    for i, p in enumerate(personas): print(f"  P{i+1}: {p[:100]}...")
    rows_out = []
    variance_log = []
    for i, (_, row) in enumerate(tqdm(scenario_df.iterrows(), total=len(scenario_df), desc=f"B5 [{country}]")):
        prompt = row.get("Prompt", row.get("prompt", ""))
        if not prompt: continue
        pref_right = bool(row.get("preferred_on_right", 1))
        cat = row.get("phenomenon_category", "default")
        # Pass 1 - mean logit aggregation
        uc = frame.format(scenario=prompt)
        fmt = chat_helper.format_query_with_suffix(uc)
        qids = tokenizer(fmt, return_tensors="pt", add_special_tokens=False).input_ids.to(device)
        p_rights_1, deltas_1 = _b5_batch_forward(model, tokenizer, chat_helper, persona_prefix_ids, qids, left_id, right_id, pad_id, cfg.logit_temperature, cfg.decision_temperature, device)
        mean_p_right_1 = float(np.mean(p_rights_1))
        variance_log.append(float(np.var(deltas_1)))
        p_spare_1 = mean_p_right_1 if pref_right else 1.0 - mean_p_right_1
        # Pass 2
        ll, rl = sf["left_lane"], sf["right_lane"]; PH = "\x00S\x00"
        sw = prompt.replace(ll,PH).replace(rl,ll).replace(PH,rl)
        ga, gb = sf.get("group_a","Group A"), sf.get("group_b","Group B")
        ...
        fmt2 = chat_helper.format_query_with_suffix(frame.format(scenario=sw))
        qids2 = tokenizer(fmt2, return_tensors="pt", add_special_tokens=False).input_ids.to(device)
        p_rights_2, _ = _b5_batch_forward(model, tokenizer, chat_helper, persona_prefix_ids, qids2, left_id, right_id, pad_id, cfg.logit_temperature, cfg.decision_temperature, device)
        mean_p_right_2 = float(np.mean(p_rights_2))
        p_spare_2 = mean_p_right_2 if not pref_right else 1.0 - mean_p_right_2
        p_spare = (p_spare_1 + p_spare_2) / 2.0
        if i < 3:
            pref_side = "RIGHT" if pref_right else "LEFT"
            print(f"\n  ── Sample {i+1} [{cat}] preferred={pref_side} ──")
            print(f"  {'Persona':<12} {'delta':>10} {'p(RIGHT)':>10}")
            print(f"  {'-'*12} {'-'*10} {'-'*10}")
            for pi_i, (delta_i, pr_i) in enumerate(zip(deltas_1, p_rights_1)):
                print(f"  P{pi_i+1:<11} {delta_i:>+10.4f} {pr_i:>10.3f}")
            print(f"  {'Consensus':<12} {np.mean(deltas_1):>+10.4f} {mean_p_right_1:>10.3f}")
            print(f"  Variance = {np.var(deltas_1):.6f}  |  p_spare={p_spare:.4f}")
            _tau = getattr(cfg, "tau_conflict", 0.001)
            mppi_word = "FIRE" if np.var(deltas_1) >= _tau else "NOT fire"
            print(f"  [NOTE] MPPI would {mppi_word} (τ={_tau})")
        rows_out.append({"country":country,"phenomenon_category":cat,"this_group_name":row.get("this_group_name","Unknown"),"n_left":int(row.get("n_left",1)),"n_right":int(row.get("n_right",1)),"preferred_on_right":int(pref_right),"p_spare_preferred":p_spare,"inter_agent_variance":variance_log[-1]})
    results_df = pd.DataFrame(rows_out)
    tau = getattr(cfg, "tau_conflict", 0.001)
    would_fire = (results_df["inter_agent_variance"] >= tau).mean()
    print(f"\n  [INFO] MPPI would have fired on {would_fire:.1%} of scenarios (τ={tau}) — SKIPPED here.")
    ...
    results_df.to_csv(os.path.join(cfg.output_dir, f"b5_persona_consensus_{country}.csv"), index=False)
    jsd = alignment.get("jsd", float("nan"))
    r   = alignment.get("pearson_r", float("nan"))
    print(f"\n[B5 {country}]  JSD={jsd:.4f}  Pearson r={r:.4f}  MPPI-would-fire={would_fire:.1%}")
    print(f"  Model AMCE: { {k: f'{v:.1f}' for k,v in model_amce.items()} }")
    print(f"  Human AMCE: { {k: f'{v:.1f}' for k,v in human_amce.items()} }")
    return {"model_amce":model_amce,"human_amce":human_amce,"alignment":alignment,"results_df":results_df,"mppi_would_fire_rate":float(would_fire)}
...
def main():
    ...
    for country in cfg.target_countries:
        ...
        result = run_b5_persona_consensus(model, tokenizer, country_df, country, personas, cfg)
    ...
    fire_rates = [r.get("mppi_would_fire_rate", float("nan")) for r in all_results]
    print(f"\n{'='*65}\n  BASELINE 5 — PersonaConsensus (No MPPI) | {len(all_results)} countries")
    print(f"  Mean JSD={np.nanmean(jsds):.4f}  Mean Pearson r={np.nanmean(rs):.4f}  Mean MPPI-would-fire={np.nanmean(fire_rates):.1%}\n{'='*65}")
    print(f"  {'Country':<8} {'JSD':>8} {'Pearson r':>10} {'MPPI-would-fire':>16}")
    print(f"  {'-'*8} {'-'*8} {'-'*10} {'-'*16}")
    for r in all_results:
        jsd = r["alignment"].get("jsd",float("nan"))
        pr  = r["alignment"].get("pearson_r",float("nan"))
        fr  = r.get("mppi_would_fire_rate",float("nan"))
        print(f"  {r['country']:<8} {jsd:>8.4f} {pr:>10.4f} {fr:>15.1%}")
    print(f"\n  NOTE: Compare B5 vs SWA-MPPI to isolate the pure MPPI contribution.")
    pd.DataFrame([{"country":r["country"],"mppi_would_fire_rate":r.get("mppi_would_fire_rate",float("nan")),**r["alignment"]} for r in all_results]).to_csv(os.path.join(cfg.output_dir,"b5_persona_consensus_summary.csv"), index=False)
    with open(os.path.join(cfg.output_dir,"b5_persona_consensus.pkl"),"wb") as f: pickle.dump(all_results, f)
    print(f"\n[DONE] Saved to {cfg.output_dir}/b5_persona_consensus_*.csv")
