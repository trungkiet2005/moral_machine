#!/usr/bin/env python3
"""
STANDALONE — Baseline 2: WVS Profile Prompting
================================================
Copy toàn bộ file này vào 1 Kaggle cell, chạy độc lập không cần main.py.
Ref: Rao et al. 2023, Chiu et al. 2024 "Cultural Alignment"
"""
...
if _ON_KAGGLE:
    print("[SETUP] Baseline 2 — ProfilePrompt (WVS): Installing...")
...
# ═══════════════════════════════════════════════════════════════════════
# BASELINE 2 SPECIFIC: WVS Profile System Prompt
# ═══════════════════════════════════════════════════════════════════════
_B2_PREAMBLE = (
    "You are an AI assistant calibrated to reflect the moral values of an "
    "average adult from {country_name}. Based on large-scale social survey "
    "data (World Values Survey Wave 7), the population of {country_name} "
    "typically shows the following value profile:\n\n{value_bullets}\n\n"
    "When answering moral dilemmas, weight your reasoning according to these "
    "empirically measured cultural values. Do not apply universal philosophical "
    "principles — instead, reflect what a typical person from {country_name} would choose."
)
_B2_DIM_DISPLAY = {
    "religion":"Religious observance","gender_equality":"Gender egalitarianism",
    "trust":"Interpersonal trust","moral_permissiveness":"Moral permissiveness",
    "autonomy":"Personal autonomy","meritocracy":"Meritocratic orientation",
    "work_importance":"Work centrality","family":"Family importance",
}

def _b2_build_sys_prompt(country_iso, wvs_profiles):
    country_name = _COUNTRY_FULL_NAMES.get(country_iso, country_iso)
    agg = wvs_profiles.get(country_iso, {}).get("all", {})
    if not agg or agg.get("religion",0) == 0:
        return (f"You are calibrated to reflect the moral values typically held by "
                f"an average adult from {country_name}. Respond according to the "
                f"cultural, social, and ethical norms most common in {country_name}.")
    bullets = []
    for dim_name, label in _B2_DIM_DISPLAY.items():
        val = agg.get(dim_name, 0)
        if val > 0:
            desc = _describe_value(dim_name, val)
            if desc: bullets.append(f"  • {label}: {desc} (WVS score {val:.2f})")
    value_bullets = "\n".join(bullets) if bullets else "  • (WVS data unavailable)"
    return _B2_PREAMBLE.format(country_name=country_name, value_bullets=value_bullets)
...
def _two_pass_p_spare(model, tokenizer, chat_helper, sys_prompt, scenario_text, pref_right, lang, left_id, right_id, pad_id, decision_temperature, device):
    frame = _PROMPT_FRAME_I18N.get(lang, _PROMPT_FRAME_I18N["en"])
    sf    = _SCENARIO_FRAME_I18N.get(lang, _SCENARIO_FRAME_I18N["en"])
    def _run(qtext, pr):
        pid  = chat_helper.build_prefix_ids(sys_prompt, device)
        fmt  = chat_helper.format_query_with_suffix(frame.format(scenario=qtext))
        qids = tokenizer(fmt, return_tensors="pt", add_special_tokens=False).input_ids.to(device)
        return _logit_p_spare(model, torch.cat([pid, qids], dim=1), left_id, right_id, pr, decision_temperature)
    ...
    return (_run(sw, not pref_right) + p1) / 2.0

def run_b2_profile_prompt(model, tokenizer, scenario_df, country, cfg, wvs_profiles):
    ...
    frame    = _PROMPT_FRAME_I18N.get(lang, _PROMPT_FRAME_I18N["en"])
    sys_prompt = _b2_build_sys_prompt(country, wvs_profiles)

    print(f"\n[B2] {country} (lang={lang})")
    print(f"  System prompt:\n{sys_prompt[:500]}")

    rows_out = []
    for i, (_, row) in enumerate(tqdm(scenario_df.iterrows(), total=len(scenario_df), desc=f"B2 [{country}]")):
        ...
        if i < 3:
            pid  = chat_helper.build_prefix_ids(sys_prompt, device)
            fmt  = chat_helper.format_query_with_suffix(frame.format(scenario=prompt))
            qids = tokenizer(fmt, return_tensors="pt", add_special_tokens=False).input_ids.to(device)
            fids = torch.cat([pid, qids], dim=1)
            full_text = tokenizer.decode(fids[0], skip_special_tokens=False)
            p_spare, p_l, p_r = _logit_p_spare(model, fids, left_id, right_id, pref_right, cfg.decision_temperature, return_raw=True)
    ...
    results_df.to_csv(os.path.join(cfg.output_dir, f"b2_profile_prompt_{country}.csv"), index=False)
    jsd = alignment.get("jsd", float("nan"))
    r   = alignment.get("pearson_r", float("nan"))
    print(f"\n[B2 {country}] JSD={jsd:.4f}  Pearson r={r:.4f}")
...
def main():
    ...
    print("[WVS] Loading profiles...")
    wvs_profiles = _load_wvs_profiles(cfg.wvs_data_path, list(_COUNTRY_FULL_NAMES.keys()))
    n_loaded = sum(1 for c in wvs_profiles if wvs_profiles[c].get("all",{}).get("religion",0)>0)
    print(f"[WVS] Loaded {n_loaded} countries")
    ...
    for country in cfg.target_countries:
        ...
        result = run_b2_profile_prompt(model, tokenizer, country_df, country, cfg, wvs_profiles)
    ...
    print(f"\n{'='*60}")
    print(f"  BASELINE 2 — ProfilePrompt (WVS) | {len(all_results)} countries")
    ...
    pd.DataFrame([{"country":r["country"],**r["alignment"]} for r in all_results]).to_csv(
        os.path.join(cfg.output_dir,"b2_profile_prompt_summary.csv"), index=False)
    with open(os.path.join(cfg.output_dir,"b2_profile_prompt.pkl"),"wb") as f:
        pickle.dump(all_results, f)
    print(f"\n[DONE] Saved to {cfg.output_dir}/b2_profile_prompt_*.csv")
