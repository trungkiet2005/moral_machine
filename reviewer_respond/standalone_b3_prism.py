#!/usr/bin/env python3
"""
STANDALONE — Baseline 3: PRISM-Style Pluralistic Prompting
============================================================
Copy toàn bộ file này vào 1 Kaggle cell, chạy độc lập không cần main.py.
6 diverse cross-demographic personas, weighted average aggregation.
Ref: Sorensen et al. 2024 "Roadmap to Pluralistic Alignment",
     Kirk et al. 2024 (PRISM dataset), Argyle et al. 2023
"""
...
if _ON_KAGGLE:
    print("[SETUP] Baseline 3 — PRISM Pluralistic: Installing...")
...
# ═══════════════════════════════════════════════════════════════════════
# ═══════════════════════════════════════════════════════════════════════
# BASELINE 3 SPECIFIC: PRISM Pluralistic Prompting
# Ref: Sorensen et al. 2024, Kirk et al. 2024
# ═══════════════════════════════════════════════════════════════════════
_B3_PRISM_PERSONAS = [
    ("YoungUrbanProgressive","You are a young urban professional in your 20s from {country_name}. You hold progressive social views, value individual rights and gender equality, and approach moral questions with secular, egalitarian reasoning. Social status or physical fitness should never determine who deserves to live.",0.18),
    ("MiddleAgedTraditional","You are a middle-aged married person from {country_name} with traditional values. Family, community bonds, and established moral norms guide your decisions. You believe in personal responsibility, duty to protect the vulnerable, and preserving social order.",0.22),
    ("ElderlyReligious","You are an elderly, deeply religious person from {country_name}. Your moral framework is shaped by your faith, deep respect for tradition, and the sanctity of all human life. You defer to religious teaching when facing hard choices.",0.15),
    ("RuralWorkingClass","You are a rural working-class adult from {country_name}. Practical common sense, community fairness, protecting the young and vulnerable, and strong local loyalty define your moral outlook. Saving more lives is generally better.",0.20),
    ("HighlyEducatedLiberal","You are a highly educated, politically liberal adult from {country_name}. You apply systematic ethical reasoning: utilitarian calculus, rights-based thinking, and concern for equality. Social status, wealth, and appearance should never determine who lives.",0.12),
    ("FemaleCaregiverEthics","You are a woman and primary caregiver from {country_name}. Ethics of care — protecting children, pregnant women, the elderly, and the physically vulnerable — is central to your moral reasoning. Nurturing relationships and protecting the weak come first.",0.13),
]

def _logit_p_spare(model, full_ids, left_id, right_id, pref_right, decision_temperature=1.0, return_raw=False):
    with torch.no_grad():
...
@torch.no_grad()
def _b3_batch_forward(model, tokenizer, chat_helper, persona_texts, scenario_text, lang,
    ...
    user_content = frame.format(scenario=scenario_text)
    prefix_ids_list = [chat_helper.build_prefix_ids(p, device) for p in persona_texts]
    query_text = chat_helper.format_query_with_suffix(user_content)
    query_ids = tokenizer(query_text, return_tensors="pt", add_special_tokens=False).input_ids.to(device)
    seqs = [torch.cat([p, query_ids], dim=1) for p in prefix_ids_list]
    ...
    return [torch.sigmoid((lr_logits[i,1]-lr_logits[i,0])/decision_temperature).item() for i in range(len(persona_texts))]

def run_b3_prism(model, tokenizer, scenario_df, country, cfg):
    ...
    sf       = _SCENARIO_FRAME_I18N.get(lang, _SCENARIO_FRAME_I18N["en"])
    persona_texts = [t.format(country_name=country_name) for _, t, _ in _B3_PRISM_PERSONAS]
    weights = np.array([w for _, _, w in _B3_PRISM_PERSONAS], dtype=np.float64)
    weights /= weights.sum()
    print(f"\n[B3-PRISM] {country} (lang={lang}) — {len(persona_texts)} diverse personas:")
    for (lbl,_,_), w in zip(_B3_PRISM_PERSONAS, weights):
        print(f"  [{w:.2f}] {lbl}")
    def _run_pass(scenario_text, pref_right):
        p_rights = _b3_batch_forward(model, tokenizer, chat_helper, persona_texts, scenario_text, lang,
                                      left_id, right_id, pad_id, cfg.logit_temperature, cfg.decision_temperature, device)
        agg_p_right = float(np.dot(weights, p_rights))
        return agg_p_right if pref_right else 1.0 - agg_p_right
    rows_out = []
    for i, (_, row) in enumerate(tqdm(scenario_df.iterrows(), total=len(scenario_df), desc=f"B3-PRISM [{country}]")):
        ...
        p1 = _run_pass(prompt, pref_right)
        ...
        p2 = _run_pass(sw, not pref_right)
        p_spare = (p1 + p2) / 2.0
        if i < 3:
            p_rights_dbg = _b3_batch_forward(model, tokenizer, chat_helper, persona_texts, prompt, lang,
                                              left_id, right_id, pad_id, cfg.logit_temperature, cfg.decision_temperature, device)
            pref_side = "RIGHT" if pref_right else "LEFT"
            print(f"\n  ── Sample {i+1} [{cat}] preferred={pref_side} ──")
            for (lbl,_,_), pr_i, w_i in zip(_B3_PRISM_PERSONAS, p_rights_dbg, weights):
                print(f"    [{lbl:28s}] p(RIGHT)={pr_i:.3f}  w={w_i:.2f}")
            print(f"  Weighted p(spare_preferred) = {p_spare:.4f}")
    ...
    results_df.to_csv(os.path.join(cfg.output_dir, f"b3_prism_{country}.csv"), index=False)
    jsd = alignment.get("jsd", float("nan"))
    r   = alignment.get("pearson_r", float("nan"))
    print(f"\n[B3-PRISM {country}]  JSD={jsd:.4f}  Pearson r={r:.4f}")
...
def main():
    ...
    for country in cfg.target_countries:
        ...
        result = run_b3_prism(model, tokenizer, country_df, country, cfg)
    ...
    pd.DataFrame([{"country":r["country"],**r["alignment"]} for r in all_results]).to_csv(
        os.path.join(cfg.output_dir,"b3_prism_summary.csv"), index=False)
    with open(os.path.join(cfg.output_dir,"b3_prism.pkl"),"wb") as f:
        pickle.dump(all_results, f)
    print(f"\n[DONE] Saved to {cfg.output_dir}/b3_prism_*.csv")
